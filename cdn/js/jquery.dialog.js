...
(function($){
	...
	window.dialog = {
		defaultParams: {
			...
			validate: function(form,alert){},
			callback: function(form,alert){
				if (this.next) {window.location.href = this.next; }
			},
		},
		...
		prompt: function(params) {
			...
			var params = $.extend(true, {}, dialog.defaultParams, params);
			var alertId = dialog.generateRandomId();
			var inputs = (params.inputs.length>0) ? params.inputs : [params.input];
			var html = [];
			for (var index in inputs) {
				var attrs = [];
				for (var attr in inputs[index]) {
					attrs.push(attr+'="'+inputs[index][attr]+'"');
				}
				html.push('<label><input '+attrs.join(' ')+'/></label>');
			}
			var inputAttributes = "";
			...
			var newAlert  = '<div class="dialog-alert" id="'+alertId+'">';
				...
				newAlert 	+= '<div class="dialog-title">' + params.title + '</div>';
				newAlert 	+= '<div class="dialog-message">' + params.message + '</div>';
				newAlert 	+= '<form action="'+params.action+'" method="get">';
				newAlert 	+= 		html.join("\n");
				newAlert 	+= '</form>';
				newAlert 	+= '<div class="dialog-close">&times;</div>';
				newAlert 	+= '<div class="dialog-confirm">' + params.button + '</div>';
			...
			confirm.bind("click.dialog", function() {
				var isValid = params.validate(alert.find('form'),alert);
				if ( !isValid ) {
				...
				params.callback(alert.find('form'),alert);
			});
			close.one("click.dialog", function() {
				params.callback(null);
			});
		...
		focusElement: function(elem, moveCursorToEnd){
			if( !elem || $(elem).attr('type')=='hidden'){ return; }
